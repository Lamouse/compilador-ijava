...
void generateOper(Exp* exp) {
	...
	if(type == Not) {
		...
		geraIndentacao();
		printf("%%%d = call noalias i8* @calloc(i64 %%%d, i64 4)\n", geraVar+3, geraVar+2);
		geraIndentacao();
		printf("%%%d = bitcast i8* %%%d to i32*\n", geraVar+4, geraVar+3);
		geraIndentacao();
		printf("store i32 %s, i32* %%%d\n", oper->params->var, geraVar+4);
		geraIndentacao();
		printf("%%%d = alloca i32*\n",geraVar+5);
		geraIndentacao();
		...
		printf("%%%d = sext i32 %%%d to i64\n", geraVar+1, geraVar);
		geraIndentacao();
		printf("%%%d = call noalias i8* @calloc(i64 %%%d, i64 1)\n", geraVar+2, geraVar+1);
		geraIndentacao();
...
void generateFunction() {
	...
	printf("declare noalias i8* @calloc(i64, i64)\n");
